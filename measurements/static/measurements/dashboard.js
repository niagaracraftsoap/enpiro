...
function renderLastRecorded(value) {
  document.querySelector("#last-recorded").textContent = value
    ? `Last reading ${formatTime(value)}`
...
async function initializeData() {
  ...
  const initialLatest = [...observations.values()].sort((left, right) => (
    new Date(left.recorded_at).getTime() - new Date(right.recorded_at).getTime()
  )).at(-1);
  ...
  renderLastRecorded(latest?.recorded_at);
  await loadRange();
}
...
async function acceptReading(reading) {
  ...
  rebuildMetricRows();
  document.querySelector("#active-range-label").textContent = describeRange();
  renderLastRecorded(reading.recorded_at);
  renderMetrics();
}
