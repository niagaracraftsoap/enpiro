...
def populate_index(apps, schema_editor):
    ...
    for term in terms.iterator(chunk_size=500):
        ...
        try:
            ...
            if lengths == [8, 2, 4, 2]:
                temperature_c = struct.unpack(">h", bytes(atoms[1]))[0] / 100
                pressure_hpa = struct.unpack(">I", bytes(atoms[2]))[0] / 100
                relative_humidity = struct.unpack(">H", bytes(atoms[3]))[0] / 100
            elif lengths == [8, 2, 2, 2]:
