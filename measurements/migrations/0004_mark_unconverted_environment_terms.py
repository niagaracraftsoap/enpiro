...
LEGACY_SHAPES = ([8, 2, 2, 2], [8, 2, 4, 2])


def mark_unconverted_environment_terms(apps, schema_editor):
    """Repair legacy readings if 0003 ran before the data was available."""
    ...
    for term in terms:
        ...
        if [len(atom) for atom in atoms] not in LEGACY_SHAPES:
            continue
...
class Migration(migrations.Migration):
    dependencies = [("measurements", "0003_substrate_only_environment")]

    operations = [
        migrations.RunPython(
            mark_unconverted_environment_terms,
